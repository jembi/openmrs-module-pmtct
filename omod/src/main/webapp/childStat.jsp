...
<openmrs:require privilege="View PMTCT pediatric tests" otherwise="/login.htm" redirect="/module/pmtct/infantResume.list?category=child&type=1&page=1" />
		...
		<c:if test="${param.type=='5'}">
			<form action="infantResume.list?category=child&type=5&page=1" method="post" style="display: inline;">
				<span><spring:message code="pmtct.month"/> : <select name="month">
					...
					<spring:message code="pmtct.menu.infantTest"/> : 
					<select name="infantTestSelect">
						<option value="1" <c:if test="${param.infantTestSelect=='1'}">selected='selected'</c:if>><spring:message code="pmtct.cf.pcrTestTitle"/></option>
						<option value="2" <c:if test="${param.infantTestSelect=='2'}">selected='selected'</c:if>><spring:message code="pmtct.cf.serology9MonthsTitle"/></option>
						<option value="3" <c:if test="${param.infantTestSelect=='3'}">selected='selected'</c:if>><spring:message code="pmtct.cf.serology18MonthsTitle"/></option>
					</select>
					&nbsp;<input type="submit" class="list_exportBt" value="<spring:message code="pmtct.tablelist.refresh"/>" title="<spring:message code="pmtct.tablelist.refresh"/>"/>
				</span>
			</form>
...
<table id="list_data">
	<tr>
		...
		<openmrs:hasPrivilege privilege='View Patient Names'>
			...
			<th class="columnHeader"><spring:message code="pmtct.general.motherNames"/></th>
		</openmrs:hasPrivilege>
		<th class="columnHeader"><spring:message code="Person.gender"/></th>
		<th class="columnHeader"><spring:message code="Person.birthdate"/></th>
		<c:if test="${param.type!='1'}">
			<th class="columnHeader"><spring:message code="pmtct.cf.scheduledVisitDateTitle"/></th>
		</c:if>
		<c:if test="${param.type=='1'}">
			<th class="columnHeader"><spring:message code="pmtct.menu.infantFeedingMethod"/></th>
			<th class="columnHeader"><spring:message code="pmtct.cpn.pcrResult"/></th>
			<th class="columnHeader"><spring:message code="pmtct.cpn.serologyTest9MonthsResult"/></th>
			<th class="columnHeader"><spring:message code="pmtct.cpn.serologyTest18MonthsResult"/></th>
		</c:if>
	</tr>
	<c:if test="${param.type!='1'}"><c:set var="columns" value="7"/></c:if>
	<c:if test="${param.type=='1'}"><c:set var="columns" value="9"/></c:if>
	<c:if test="${empty numberOfPages}"><tr><td colspan="${columns}"><center><spring:message code="pmtct.tablelist.empty"/></center></td></tr></c:if>
...
<!-- <div id="chartHolder" style="display: none;">
	...
	<div id="graph"><img src="chart.htm?chart=infant&type=1" width="400" height="300" /></div>	
		<ul>
			<li><a id="graph_1" href="javascript:void(0)">[ <spring:message code="pmtct.menu.infantTest"/> : <spring:message code="pmtct.menu.infantFeedingMethod"/> ]</a></li>
			<li><a id="graph_2" href="javascript:void(0)">[ <spring:message code="pmtct.menu.infantTest"/> : <spring:message code="pmtct.menu.resultOfHivTestAfterPcr"/> ]</a></li>
			<li><a id="graph_3" href="javascript:void(0)">[ <spring:message code="pmtct.menu.infantTest"/> : <spring:message code="pmtct.menu.resultOfHivTestAfterSer9M"/> ]</a></li>
			<li><a id="graph_4" href="javascript:void(0)">[ <spring:message code="pmtct.menu.infantTest"/> : <spring:message code="pmtct.menu.resultOfHivTestAfterSer18M"/> ]</a></li>
		</ul>	
	</div>
</div> -->
